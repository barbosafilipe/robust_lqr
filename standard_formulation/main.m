...
% Variaveis globais
    warning off
    global Ts Ttotal v x_ref m n p q l Q R H Ef Eg mi c c1 c2 c3 h1 l2 a2 b2 m2 cs1 cq1 a1 m1 J1 J2 P0 F_ref G_ref F_inc G_inc
    warning  on
...
%% H_infinity
[u_h,x_h,erro_h] = h_infinity(x0,N,x_ref,lim_sup,lim_inf,Ts,max_rate);
...
for k = 1:N-1
    ...
    x(:,k+1) = (F_inc)*x(:,k) + (G_inc)*u(:,k);
        ...
        velX(k) = v(k) - x(1,k)*psi(k+1);
        velY(k) = x(1,k) + v(k)*psi(k+1);
...
% findig the position fot H_infinity controller
        ...
        velX_h(k) = v(k) - x_h(1,k)*psi_h(k+1);
        velY_h(k) = x_h(1,k) + v(k)*psi_h(k+1);
...
%     annotation('arrow',[0.69 0.76],[0.5 0.58]) %put an arrow
    legend({'Reference','RLQR'},'FontSize',13)
...
% Subplot state vector
    ...
    axis([t(1) t(end) -2 2])
    legend({'H_{\infty}','RLQR'},'Location','NorthEast','FontSize',13)
    ylabel('$\dot{\psi_{1}}$ (rad/s)','Interpreter','latex','FontSize',14.5)
    title('\textbf{Yaw rate ($\dot{\psi_{1}}$)}','Interpreter','latex','FontSize',14.5)
    grid on
    ...
    legend({'H_{\infty}','RLQR'},'Location','NorthEast','FontSize',13)
    ylabel('$\dot{\phi_{1}}$ (rad/s)','Interpreter','latex','FontSize',14.5)
    title('\textbf{Articulation angle rate ($\dot{\phi_{1}}$)}','Interpreter','latex','FontSize',14.5)
    grid on
    ...
    plot(t,x(4,:),'-.k','LineWidth',1.5)
    legend({'H_{\infty}','RLQR'},'Location','NorthEast','FontSize',13)
    ylabel('$\phi_{1}$ (rad)','Interpreter','latex','FontSize',14.5)
    title('\textbf{Articulation Angle ($\phi_{1}$)}','Interpreter','latex','FontSize',14.5)
    grid on
